...
def preprocess_data():
    ...
    excel_path = os.path.join(script_dir, '2026.xlsx')        #change name of Excel document for input
...
# MAIN
# -----------------------------
def check_constraints(TA_len, course_len, task_len, df_TAs, df_TA_req, df_times, t2c, assigned_TAs, TA_total_time, c_taught, solver, taken_courses, preferences, weights, switched_c, TIME_RATIO, cons,is_active):
    ...
    time_per_task(TA_len, course_len, task_len, df_times, t2c, solver, TIME_RATIO, weights,is_active,MIN_CHUNK_HOURS,df_TA_req)
...
def save_result(df_TAs, df_ass, df_c, happ, df_TA_req, df_ta_task):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    excel_path = os.path.join(script_dir, '2027_Z3.xlsx')          # Change name of Excel document for output
...
def main():
    ...
    check_constraints(TA_len, course_len, task_len, df_TAs, df_TA_req, df_times, t2c, assigned_TAs, TA_total_time, c_taught, solver, taken_courses, preferences, weights, switched_c, TIME_RATIO, cons,is_active)
