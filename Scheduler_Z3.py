...
def load_cons(cons):
    ...
    return ALLOWED_DEV_H, ALLOWED_DEV_S, MAX_COURSES_H, MAX_COURSES_S, MAX_TAS_H, MAX_TAS_S, MAX_SWITCH_H, MAX_SWITCH_S, MIN_CHUNK_HOURS, ALLOWED_DEV_M
...
def switching_courses(TA_len, course_len, df_TAs, df_TA_req, solver, c_taught, taken_courses, weights, switched_c,
    ...
    for TA in range(TA_len):
        ...
        for c in range(course_len):
            ...
            switch_c.append(If(And(c_taught[TA][c], BoolVal(taken_courses.iloc[last_i, last_j] == 0)), 1, 0))
...
def time_per_task(TA_len, course_len, task_len, df_times, t2c, solver, TIME_RATIO, weights, MIN_CHUNK_HOURS, df_TA_req):
    target_min_chunk = int(MIN_CHUNK_HOURS * TIME_RATIO)
    ...
    for c in range(course_len):
        for t in range(task_len):
            ...
            #
            # for TA in range(TA_len):
            #     # Use Indicator Constraint (Switch)
            #     is_active = Bool(f"active_{TA}_{c}_{t}")
            #
            #     # If active: must be at least the calculated min_assign #todo this is different
            #     solver.add(t2c[TA][c][t] >= If(is_active, min_assign, 0))
            #
            #     # If active: must be at most the total task time
            #     solver.add(t2c[TA][c][t] <= If(is_active, task_time, 0))
...
def TA_deviation(TA_len, df_TAs, TA_total_time, solver, TIME_RATIO, ALLOWED_DEV_H, ALLOWED_DEV_S, weights):
    ...
    for TA in range(TA_len):
        if df_TAs['Year'].iloc[TA] >= 5:
            solver.add_soft(TA_total_time[TA] == int(df_TAs['Target'].iloc[TA] * TIME_RATIO),
                            weight=int(weights['Weight'].iloc[1]))
...
def taught_courses(TA_len, course_len, task_len, t2c, c_taught, solver, MAX_COURSES_H, MAX_COURSES_S, MAX_TAS_H,
                   MAX_TAS_S, taken_courses, preferences, weights, df_TA_req, assigned_TAs):
    ...
    for TA in range(TA_len):
        solver.add_soft(Sum([If(c_taught[TA][c], 1, 0) for c in range(course_len)]) <= MAX_COURSES_S,
                        weight=int(weights['Weight'].iloc[3]))
    ...
    for c in range(course_len):
        ...
        for t in range(task_len):
            ...
            solver.add_soft(assigned_TAs[t][c] <= (int(df_TA_req.iloc[t, c + 1]) + MAX_TAS_S),
                            weight=int(weights['Weight'].iloc[4]))
...
def find_sol(TA_len, course_len, task_len, df_TAs, df_TA_req, TA_total_time, assigned_TAs, solver, TIME_RATIO, c_taught,
             taken_courses, preferences, t2c):

    m = solver.model()

    # Find TA time results
    df_TAs['Assigned'] = [m.evaluate(time_expr).as_long() / TIME_RATIO for time_expr in TA_total_time]
    df_TAs['Deviation'] = [(m.evaluate(time_expr).as_long() / TIME_RATIO) - df_TAs['Target'].iloc[TA] for TA, time_expr
...
def solve_with_progress(TA_len, course_len, task_len, df_TAs, df_TA_req, TA_total_time, assigned_TAs, solver,
                        TIME_RATIO, c_taught, taken_courses, preferences, t2c):
    # Run solver
    ...
    df_TAs, df_ass, df_c, happ, df_ta_task = find_sol(TA_len, course_len, task_len, df_TAs, df_TA_req,
                                                      TA_total_time,
                                                      assigned_TAs, solver, TIME_RATIO, c_taught, taken_courses,
                                                      preferences, t2c)
...
# MAIN
# -----------------------------
def check_constraints(TA_len, course_len, task_len, df_TAs, df_TA_req, df_times, t2c, assigned_TAs, TA_total_time,
                      c_taught, solver, taken_courses, preferences, weights, switched_c, TIME_RATIO, cons):

    (ALLOWED_DEV_H, ALLOWED_DEV_S, MAX_COURSES_H, MAX_COURSES_S, MAX_TAS_H, MAX_TAS_S, MAX_SWITCH_H, MAX_SWITCH_S,
                      MIN_CHUNK_HOURS, ALLOWED_DEV_M) = load_cons(cons)
    switching_courses(TA_len, course_len, df_TAs, df_TA_req, solver, c_taught, taken_courses, weights, switched_c,
                      MAX_SWITCH_H, MAX_SWITCH_S)
    time_per_task(TA_len, course_len, task_len, df_times, t2c, solver, TIME_RATIO, weights,MIN_CHUNK_HOURS, df_TA_req)
    TA_deviation(TA_len, df_TAs, TA_total_time, solver, TIME_RATIO, ALLOWED_DEV_H, ALLOWED_DEV_S, weights)
    taught_courses(TA_len, course_len, task_len, t2c, c_taught, solver, MAX_COURSES_H, MAX_COURSES_S, MAX_TAS_H,
                   MAX_TAS_S, taken_courses, preferences, weights, df_TA_req, assigned_TAs)
...
def main(year):
    ...
    check_constraints(TA_len, course_len, task_len, df_TAs, df_TA_req, df_times, t2c, assigned_TAs, TA_total_time,
                      c_taught, solver, taken_courses, preferences, weights, switched_c, TIME_RATIO, cons)

    start_time = tm()

    df_TAs, df_ass, df_c, happ, df_ta_task = solve_with_progress(TA_len, course_len, task_len, df_TAs, df_TA_req,
                                                                 TA_total_time, assigned_TAs, solver, TIME_RATIO,
                                                                 c_taught, last_year_courses, preferences, t2c)

    end_time = tm()-start_time

    # Save
    save_result(df_TAs, df_ass, df_c, happ, df_TA_req, df_ta_task,year)
