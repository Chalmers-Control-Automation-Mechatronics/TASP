...
def create_matrices(TA_len, course_len, task_len):
    ...
    is_active = [[[Bool(f"active_{TA}_{c}_{t}") for t in range(task_len)] for c in range(course_len)] for TA in
                 range(TA_len)]
    ...
    return t2c, TA_total_time, c_taught, assigned_TAs, switched_c, is_active
...
def time_per_task(TA_len, course_len, task_len, df_times, t2c, solver, TIME_RATIO, weights,
                  is_active, MIN_CHUNK_HOURS, df_TA_req):
    for c in range(course_len):
        for t in range(task_len):
...
def solve_with_progress(TA_len, course_len, task_len, df_TAs, df_TA_req, TA_total_time, assigned_TAs, solver,
    ...
    df_TAs, df_ass, df_c, happ, df_ta_task = find_sol(TA_len, course_len, task_len, df_TAs, df_TA_req, TA_total_time,
...
def check_constraints(TA_len, course_len, task_len, df_TAs, df_TA_req, df_times, t2c, assigned_TAs, TA_total_time,
                      c_taught, solver, taken_courses, preferences, weights, switched_c, TIME_RATIO, cons, is_active):
    ALLOWED_DEV_H, ALLOWED_DEV_S, MAX_COURSES_H, MAX_COURSES_S, MAX_TAS_H, MAX_TAS_S, MAX_SWITCH_H, MAX_SWITCH_S, MIN_CHUNK_HOURS = load_cons(
        cons)
    switching_courses(TA_len, course_len, df_TAs, df_TA_req, solver, c_taught, taken_courses, weights, switched_c,
                      MAX_SWITCH_H, MAX_SWITCH_S)
    time_per_task(TA_len, course_len, task_len, df_times, t2c, solver, TIME_RATIO, weights, is_active, MIN_CHUNK_HOURS,
                  df_TA_req)
...
def main(year):
    ...
    t2c, TA_total_time, c_taught, assigned_TAs, switched_c, is_active = create_matrices(TA_len, course_len, task_len)
